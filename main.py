...
MY_EMAIL = "blackletter.calli@gmail.com"
MY_PASSWORD = "poukecfdboaimxhb"
...
def register():
    ...
    if form.validate_on_submit():
        ...
        hash_salt_password = generate_password_hash(form.password.data, method="pbkdf2", salt_length=10)
        new_user = User(
            name=form.name.data,
            email=user_email,
    ...
    if request.method == "POST":
        ...
        with smtplib.SMTP("smtp.gmail.com", port=587) as connection:
            ...
            connection.sendmail(
                from_addr=MY_EMAIL,
                to_addrs=MY_EMAIL,
                msg=f"Subject:Someone has query on MY_BLOGS\n\n"
                    f"Name: {name}\nEmail: {email}\nPhone: {phone}\nMessage: {message}".encode("utf-8"))
        return render_template("contact.html", msg_sent=True, is_logged=current_user.is_authenticated)
    if request.method == "GET":
        return render_template("contact.html", msg_sent=False, is_logged=current_user.is_authenticated)
